...
## Branching
We're using the [Gitflow](https://www.atlassian.com/git/tutorials/comparing-workflows/gitflow-workflow) branching model. The `main` branch is the stable one, while the `develop` branch is the one where we merge all the features and bug fixes.

`main` and `develop` are protected branches, meaning that you can't push directly to them. You'll have to open a PR to merge your changes.
