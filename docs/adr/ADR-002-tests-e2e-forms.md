...
1. Repository
    ...
    2. Separate repository
       Pros: - It could have be better if we decide to test more than LEAV
...
3. Frequency
    ...
    2. each PR
       Pros: - Nothing is merged if tests are broken
       Cons: - Slower CI
...
4. How to handle data
    ...
    2. Abstract (ex: tree, linked field)
       Pros: - Independant from implementation

5. How to write e2e tests
    - BDD
      Cons: - We don't have enough reasons to choose to write test with this pattern
    - PageObjectModel
      Pros: - We have written tests using this pattern in the existing e2e test repository.
