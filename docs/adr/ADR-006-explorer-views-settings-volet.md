...
| Multi-attribute sort & filters in `viewV2ToSerializedView` | Shipped: pinned `sort` (LEAVC-851) and pinned lean `filters` (LEAVC-810) are serialized. The sort `pinned` field was later renamed `activated` (LEAVC-588 follow-up) — same semantics.                   |
