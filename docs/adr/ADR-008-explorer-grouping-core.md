...
- L'arbre ne nécessite **aucun** nouveau champ backend : compteurs **par nœud** via `recordsGroups`,
  **structure** via `treeNodeChildren`, **cumul** (nœud + descendants) sommé **côté front**. Le
  fetch des records d'un nœud d'attribut arbre se fait via un chemin à 3 segments **confirmé**
  (`<attr>.<libraryDuNœud>.id` `EQUAL` `<record id du nœud>`) — cf. _Open points_.
...
| **Fetch des records d'un nœud d'attribut arbre** — **RÉSOLU** : chemin à 3 segments `<attr>.<libraryDuNœud>.id` avec `EQUAL` sur le **record id du nœud**, ex. `records(users, [{field: "user_groups.users_groups.id", condition: EQUAL, value: "1"}])` → 21. Le `field` nu ou à 2 segments ne matche pas ; `CLASSIFIED_IN` non plus (appartenance propre). La library du milieu = celle du nœud (`recordsGroups` la fournit via `value.record.whoAmI.library.id`). | Résolu (2026-07-01)    |
