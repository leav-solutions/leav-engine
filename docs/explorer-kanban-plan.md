...
12. **Désignation de l'axe** : quand `viewType === kanban`, permettre de marquer **un attribut déjà dans la liste
    ...
    > édite pas). **Décision (cf. §9-7) : gating reporté après PR-3.** `KanbanAxisSelector` est donc affiché
    > **sans condition de permission** en l'état — le gating sur `canManageViews` (ou la restriction à une
    > sélection rendue disponible par l'admin) reste à faire, en attente des maquettes (la gestion du
    > regroupement changera peut-être d'onglet dans le volet).
...
- describe "Vue Kanban — pagination par colonne"
  - affiche les 10 premières cartes d'une colonne et son compteur total
  - « Voir plus » charge les 10 cartes suivantes de la colonne
  - « Voir plus » disparaît quand toutes les cartes de la colonne sont chargées
```
...
6. **Drop vers « Sans valeur » = autorisé, gaté par le workflow** (PR-3). L'effacement est une
   transition modélisée côté core : `allowedDependentValues` peut contenir `{nodeId: null}` (exclu
   d'office pour un attribut `required`). La colonne « Sans valeur » n'est donc droppable que si la
   transition vers `null` est permise depuis le nœud courant ; l'écriture passe par
   `value: null` + `deleteEmpty`.
7. **Gating `canManageViews` du sélecteur d'axe** (§5.5) : **reporté après PR-3** — en attente des
   maquettes ; la gestion du regroupement changera peut-être d'onglet dans le volet.
