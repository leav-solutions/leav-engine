...
export const useDeactivateMassAction = ({
            ...
            cache.modify({
                fields: {
                    records: prev => ({
                        ...prev,
                        totalCount:
                            view.massSelection === MASS_SELECTION_ALL ? 0 : prev.totalCount - view.massSelection.length
                    })
                },
                broadcast: false
            });
    ...
    const _deactivateMassAction: IMassActions = useMemo(
            ...
            callback: massSelectionFilter =>
                KitModal.confirm({
                    ...
                    onOk: async () => {
                        ...
                        if (view.massSelection === 'all' || view.massSelection.length === allVisibleKeys.length) {
                            await refetch({
                                pagination: {
                                    limit: view.pageSize,
                                    offset: 0
        ...
        [t, deactivateRecordsMutation, view.massSelection, dispatch, libraryId, allVisibleKeys]
    );
