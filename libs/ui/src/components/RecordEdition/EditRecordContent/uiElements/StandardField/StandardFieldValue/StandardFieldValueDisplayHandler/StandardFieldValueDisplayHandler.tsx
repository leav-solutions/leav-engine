...
const StandardFieldValueReadToggled = styled(StandardFieldValueRead)<{$hidden: boolean}>`
    display: ${({$hidden}) => ($hidden ? 'none' : 'block')};
`;

const FormItemToggled = styled(Form.Item)<{$hidden: boolean}>`
    display: ${({$hidden}) => ($hidden ? 'none' : 'block')};
`;
    ...
    return (
        <>
            <StandardFieldValueRead
                ...
                onClick={_handleClickOnReadValue}
                style={{display: mustDisplayReadValue ? 'block' : 'none'}}
            />
            <Form.Item
                name={attribute.id}
                rules={[
