    ...
    describe('Attribute with multiple values', () => {
        describe('For link attribute', () => {
            ...
            test('Should display list of avatar as default', async () => {
                ...
                // The "+N" avatar is generated by antd from the group's own size, never from its
                // children: without a size on the group it falls back to antd's default and dwarfs
                // the rest of the group.
                const avatarWidths = ['RA', 'RB', 'RC', 'RD', '+2'].map(
                    text => screen.getByText(text).closest<HTMLElement>('.ant-avatar')?.style.width,
                );
                expect(avatarWidths).toEqual(['24px', '24px', '24px', '24px', '24px']);
            });
