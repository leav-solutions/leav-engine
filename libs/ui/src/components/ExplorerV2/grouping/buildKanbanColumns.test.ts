import {buildKanbanColumns, NO_AXIS_VALUE_COLUMN_ID} from './buildKanbanColumns';
import {type IKanbanAxisNode} from './_types';
...
const axisNodes: IKanbanAxisNode[] = [
    ...
    {recordId: 'validated', nodeId: 'node-validated', libraryId: 'statuses', label: 'Validated', color: '#0f0'},
    {recordId: 'archived', nodeId: 'node-archived', libraryId: 'statuses', label: 'Archived', color: null},
];
...
describe('buildKanbanColumns', () => {
    ...
    it('carries the axis node id on each column (null on the no-value column)', () => {
        const columns = buildKanbanColumns({
            records: [card('a', null), card('b', 'validated')],
            groupByAttributeId: AXIS,
            axisNodes,
        ...
        expect(columns.map(col => col.nodeId)).toEqual([null, 'node-draft', 'node-validated', 'node-archived']);
    });
