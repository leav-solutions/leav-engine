...
- Always `await` the async utilities (`waitFor`, `waitForElementToBeRemoved`, `findBy*`). A floating `waitFor` keeps polling after the test ended, once RTL cleanup has emptied the DOM: its assertion then fails forever and rejects outside any test. Vitest reports it as an _unhandled rejection_ and exits `1` while every test is green (`321 passed | 1 error`), and whether it happens at all depends on machine load — hence green locally, flaky in CI.
