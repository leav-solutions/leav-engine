...
| [ADR-006 — Explorer view settings panel](docs/adr/ADR-006-explorer-views-settings-volet.md) | État dans le contexte application-settings (pas en search params), feature flag `enableViewSettings`, communication inter-panneaux via `message-to-panel` (`usePanelMessenger`), Explorer agnostique d'app-studio, props contrôlées `currentView` (+ `APPLY_SERIALIZED_VIEW`) et `loadedViewId`. |
