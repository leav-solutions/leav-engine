...
class BinDragLayer extends Component {
  ...
  renderItem = (type, item) => {
    if (item) {
      const connectionState = item.connectionState === undefined ? 'neutral' : item.connectionState ? 'connected' : 'disconnected';
      const colorTypeDictionnary = item.colorTypeDictionnary
      const width = item.width
      return <div style={{pointerEvents: "none", width: `${width}px`}}>
                <ALCCard
                  key={item.id}
                  id={`${item.id}`}
                  action={item.action}
                  moveCard={() => {}}
                  findCard={() => {return {index: item.id}}}
                  origin={item.origin}
