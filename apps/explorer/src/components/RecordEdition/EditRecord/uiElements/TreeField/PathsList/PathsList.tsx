...
const BreadcrumbItem = styled(Breadcrumb.Item)`
    ...
    // First line needs room for field label
    ${BreadcrumbRoot}:first-child & {
        padding-top: 1em;
    }
`;

function PathsList({paths}: IPathsListProps): JSX.Element {
                ...
                {parents.map(ancestor => {
                    return (
                        <BreadcrumbItem key={ancestor.record.id}>
                            <RecordCard record={ancestor.record.whoAmI} size={PreviewSize.small} />
                            <Breadcrumb.Separator>{'>'}</Breadcrumb.Separator>
                        </BreadcrumbItem>
                    );
                })}
                <BreadcrumbItem key={element.record.id}>
                    <RecordCard record={element.record.whoAmI} size={PreviewSize.small} />
                </BreadcrumbItem>
            </BreadcrumbRoot>
        );
    };
