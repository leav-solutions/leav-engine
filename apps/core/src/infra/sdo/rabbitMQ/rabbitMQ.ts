...
export default function rabbitMQ({'core.infra.amqpService': amqpService, config}: IRabbitMQDeps): IRabbitMQ {
    ...
    const getLeavDataEventChannel = async () => {
        if (!_leavDataEventChannel) {
            _leavDataEventChannel = await amqpService.consumer.connection.createConfirmChannel();
    ...
    const getSDOExportChannel = async () => {
        if (!_sdoExportChannel) {
            _sdoExportChannel = await amqpService.publisher.connection.createConfirmChannel();
    ...
    const getSDOImportChannel = async () => {
        if (!_sdoImportChannel) {
            _sdoImportChannel = await amqpService.consumer.connection.createConfirmChannel();
