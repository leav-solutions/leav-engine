...
describe("dbUtils", () => {
  ...
  describe("findCoreEntity", () => {
    ...
    test("Find core entity without filters", async () => {
      ...
      expect(
        mockDbServ.execute.mock.calls[0][0].query.bindVars["@value0"]
      ).toBe("core_trees");
      expect(
        mockDbServ.execute.mock.calls[0][0].query.bindVars
      ).toMatchSnapshot();
    ...
    test("Filter with a LIKE on ID", async function () {
      ...
      expect(mockDbServ.execute.mock.calls[0][0].query.query).toMatch(
        /(FILTER LIKE){1}/
      );
      expect(mockDbServ.execute.mock.calls[0][0].query.query).toMatchSnapshot();
      expect(
    ...
    test("Should limit results", async function () {
      ...
      expect(mockDbServLimit.execute.mock.calls[0][0].query.query).toMatch(
        /LIMIT/
      );
      expect(
        mockDbServLimit.execute.mock.calls[0][0].query.query
      ).toMatchSnapshot();
      expect(
        mockDbServLimit.execute.mock.calls[0][0].query.bindVars
      ).toMatchSnapshot();
    });
    ...
    test("Should convert ID key when sorting", async function () {
      ...
      expect(
        mockDbServLimit.execute.mock.calls[0][0].query.bindVars.value1
      ).toBe("_key");
    });
    ...
    test("Should use custom filter condtion supplied", async () => {
      ...
      expect(mockDbServCustom.execute.mock.calls[0][0].query.query).toMatch(
        /(CUSTOM FILTER){1}/
      );
    });
  });
  ...
  describe("migrate", () => {
    test("Run core migrations", async () => {
      ...
      (loadMigrationFile as jest.FunctionLike) = global.__mockPromiseMultiple([
        file1,
        file2,
      ]);

      // Mock migration files reading
      (readdirSync as jest.FunctionLike) = jest
        .fn()
        .mockReturnValue(["000.ts", "001.ts"]);
