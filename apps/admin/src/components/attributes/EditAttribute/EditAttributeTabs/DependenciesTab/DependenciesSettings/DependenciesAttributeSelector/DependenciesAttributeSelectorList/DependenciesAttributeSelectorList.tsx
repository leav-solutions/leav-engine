...
interface IDependenciesAttributeSelectorListProps {
    attributes: GET_ATTRIBUTES_attributes_list[];
    onSelect: (selectedAttribute: GET_ATTRIBUTES_attributes_list) => void;
}

function DependenciesAttributeSelectorList({
    attributes,
    onSelect,
}: IDependenciesAttributeSelectorListProps): JSX.Element {
    const {lang} = useLang();
        ...
        <List divided aria-label="attribute-selector-list">
            {!attributes.length && (
                <Message info size="tiny">
                    {t('attributes.dependencies.settings.no_attributes_to_add')}
                </Message>
            )}
...
export default DependenciesAttributeSelectorList;
