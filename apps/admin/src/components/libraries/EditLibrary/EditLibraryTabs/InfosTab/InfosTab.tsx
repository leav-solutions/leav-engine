...
function InfosTab({library, readonly}: IInfosTabProps): JSX.Element {
    ...
    const [saveLibrary, {error: errorSave}] = useSaveLibraryMutation({
    ...
    const _handleSubmit = async libData => {
            ...
            recordIdentityConf:
                libData.recordIdentityConf !== null
                    ? {
                          ...
                          parentContext: libData.recordIdentityConf.parentContext,
                      }
                    : null,
        };
        await saveLibrary({
        ...
        if (isNewLib) {
            navigate('/libraries/edit/' + libData.id, {replace: true});
        }
    };
