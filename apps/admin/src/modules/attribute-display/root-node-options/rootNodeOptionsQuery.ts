...
export const ROOT_NODE_OPTIONS_MAX_DEPTH = 10;

const NODE_FRAGMENT = `
    fragment TreeContentNodeInfo on TreeNode {
        id
        childrenCount
        record {
            id
...
const _nestChildren = (depth: number): string => {
    ...
    for (let level = 0; level < depth; level++) {
        selection = `children { ...TreeContentNodeInfo ${selection} }`;
    }
 ...
 */
export const rootNodeOptionsQuery = (depth = ROOT_NODE_OPTIONS_MAX_DEPTH) => gql`
    ${NODE_FRAGMENT}
    query getRootNodeOptions($treeId: ID!) {
...
export interface ITreeContentNode {
    id: string;
    childrenCount?: number | null;
    record: {id: string; whoAmI: {id: string; label?: string | null; library: {id: string}}};
    children?: ITreeContentNode[] | null;
}

export interface IRootNodeOptionsQueryData {
    treeContent: ITreeContentNode[];
}

export interface IRootNodeOptionsQueryVariables {
    treeId: string;
}
