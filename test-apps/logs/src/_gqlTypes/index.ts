...
export type CreateRecordMutation = { createRecord: { record?: { id: string } | null } };
...
export type GetLogsQuery = { logsResult?: { total: number, list: Array<{ action?: LogAction | null, time: number }> } | null };
...
export const CreateRecordDocument = gql`
    mutation CreateRecord($library: ID!) {
...
export const GetLogsDocument = gql`
    query GetLogs($filters: LogFilterInput, $sort: LogSortInput, $pagination: Pagination) {
  logsResult: logs(filters: $filters, sort: $sort, pagination: $pagination) {
    list: logs {
      action
      time
    }
...
export function getSdk(client: GraphQLClient, withWrapper: SdkFunctionWrapper = defaultWrapper) {
  return {
    ...
    GetLogs(variables?: GetLogsQueryVariables, requestHeaders?: GraphQLClientRequestHeaders, signal?: RequestInit['signal']): Promise<GetLogsQuery> {
      return withWrapper((wrappedRequestHeaders) => client.request<GetLogsQuery>({ document: GetLogsDocument, variables, requestHeaders: { ...requestHeaders, ...wrappedRequestHeaders }, signal }), 'GetLogs', 'query', variables);
    }
  };
}
